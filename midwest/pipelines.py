...
class MidwestPipeline:

    # Database setup
    conn = sqlite3.connect('midwest.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `midwest`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM midwest WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO midwest (date, title, link, content)"
